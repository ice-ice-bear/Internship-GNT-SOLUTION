...
def _set_variable(**kwargs):
    ...
    # days_before = today - datetime.timedelta(days=35)
...
with DAG(f'answersheet_{road}',
         ...
         tags = ["yolov5"],
         start_date=pendulum.datetime(2023, 8, 8 ,tz="Asia/Seoul"),
    ...
    make_datasheet = BashOperator(
        ...
        source yolov5_env/bin/activate
        cd /home/gnt/yolov5
        python3 ms_datasheet.py --date {{ ti.xcom_pull(task_ids='set_variable', key = 'date') }}
        """
        )
    
    set_variable >> copy_side_image >> run_object_detection >> make_datasheet
