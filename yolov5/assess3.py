...
def main():
    """ 
    Title: Accesment_Automation(ver3.0)
    Platform: os, pandas 
    Version: 0.3.0
    Created by srl(SeungRyul Lee), 2023.06.07
    ...
    Update: Update: Changed the 'seat1' answersheet condition
    """
...
def generate_answer_sheet(folder_path, set_num):
    ...
    for i in range(0, len(df), 3):
        ...
        for j, group in enumerate(['_01_']):
            values = [row1[group], row2[group]]
            
            if values[0] == 'emp' and values[1] != 'emp':
                df.at[i, 'answer1'] = values[1]
            elif values[0] == 'emp' and values[1] == 'emp':
                df.at[1, 'answer1'] = ''
            elif values[0] == 'O' or 'X':
