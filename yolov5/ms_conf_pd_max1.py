...
def ms_answersheet(date):
    ...
    csv_file_path = f"C:/Users/Gnt_Safe_Port_DT/Desktop/ms_{date}_answersheet_max1.csv"
    ...
    for file_name in tqdm(all_files):
        ...
        with open(file_path, 'r') as file:
            for line in file:
                ...
                conf = float(line.split()[4])  # Convert confidence to float
    ...
    for desired_filename, file_info in desired_filenames.items():
        ...
        # Rearrange the seat and seat_conf data into the desired order
        seat_data = [f'seat{i+1}={seat_list[i]}, seat{i+1}_conf={seat_conf_list[i]}' for i in range(12)]
        
        data.append({
            'filename': desired_filename,
            'seated': len(unique_labels),
            **{f'seat{i+1}': seat_data[i] for i in range(12)}
        })
